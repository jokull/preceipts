...
pub use preceipts_proto::socket_path;
...
pub struct Daemon {
    ...
    /// Whether the TLS proxy is actually listening.
    ///
    /// Load-bearing for what we tell people the URL is. Without the local CA
    /// there is no proxy, and a hostname that resolves to loopback with
    /// nothing bound on the proxy port is a connection refused — so printing
    /// `https://api.stand.localhost:8443` in that state is a lie the status
    /// table tells with a straight face. Set at boot and cleared if the TLS
    /// config fails to build.
    pub https: Arc<std::sync::atomic::AtomicBool>,
...
impl Daemon {
    pub async fn run(
        ...
        let daemon = Arc::new(Daemon {
            ...
            https: Arc::new(std::sync::atomic::AtomicBool::new(
                !hostnames.is_empty() && ca::is_installed(),
            )),
            proxy_port,
...
fn build_proc_status(daemon: &Daemon, id: &str, p: &Proc) -> ProcStatus {
    ...
    ProcStatus {
        ...
        url: match (
            daemon.hostnames.get(id),
            daemon.https.load(std::sync::atomic::Ordering::Relaxed),
        ) {
            (Some(host), true) => Some(public_url(host, daemon.proxy_port)),
            // No proxy is listening, so the hostname is not an address. The
            // allocated port is — and saying so is how "what is around me"
            // stays answerable before anything privileged is installed.
            _ => daemon
                .allocated_ports
                .get(id)
