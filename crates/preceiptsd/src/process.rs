...
impl Proc {
    ...
    pub fn send_signal(&self, signal: i32) -> Result<()> {
        ...
        if st.is_terminal() {
            return Err(anyhow!("process is already {}", st.as_str()));
        }
        let pid = self
            .pid
            .lock()
            .ok_or_else(|| anyhow!("process has no pid yet"))?;
        // Send to the process group so child workers receive reload/terminate
        // signals consistently with daemon shutdown.
        let rc = unsafe { libc::kill(-pid, signal) };
        if rc == 0 {
            Ok(())
        } else {
            Err(std::io::Error::last_os_error()).context("kill process group")
        }
    }
}
