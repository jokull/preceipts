...
pub async fn receive(service: &str, keychain: Option<&str>) -> Result<()> {
    ...
    for (k, v) in &payload.secrets {
        secrets::set(service, k, v, keychain).with_context(|| format!("store {k}"))?;
        println!("    + {k}");
    }
    println!();
...
pub async fn send(
    ...
    for k in &keys {
        match secrets::get(service, k, keychain)? {
            Some(v) => {
